...
export default new Vuex.Store({
  ...
  actions: {
    ...
    onSearch: function({commit}, query) {  // 구조분해할당 :context안에 commit있어서 commit만 가져오게 하는것
      axios.get(API_URL, {
        params: {
          part: 'snippet',
          q: query,
      ...
      .then(res => {
        ...
        commit('ADD_SEARCH_RESULT', res.data.items)  // commit만 들고올 수 있어서 굳
  ...
  getters: {
    ...
    videoTitle: function (state) {
      if (state.selectedVideo) {
        ...
        return _.unescape(title)  // 글자 깨지는 것 방지
      }
      return ''
    },
    // 상세 설명
